...
if(isset($_POST['con_payment'])){
    ...
    $nid=$cust_nid;
    $tr_num=$_POST['tr_num'];
    $tr_method=$pay_option;
    $tr_date=date("Y-m-d H:i:s");
    $p_id=$pid;
    $pay_status='Complete';
    
    
    $inst="INSERT INTO con_payment(Cust_Name,Cust_nid,tr_num,tr_method,tr_date,p_id,pay_status) VALUES
         ('$name','$nid','$tr_num','$tr_method','$tr_date','$p_id','$pay_status')"; 
    
    if(mysqli_query($cn,$inst)){
        $update="UPDATE user_order SET Payment_status='$pay_status' WHERE U_Name='$name' AND Pid='$p_id' ";
        $update_admin="UPDATE admin_order_details SET confirm_status='$pay_status' WHERE Admin_name='$ad' AND Select_pid='$p_id' ";
        mysqli_query($cn,$update);
        mysqli_query($cn, $update_admin);
        $_SESSION['p_id']=$p_id;
...
$sql_a="SELECT * FROM log1 WHERE Name='$ad' ";
$result_a=mysqli_query($cn,$sql_a);
 if(mysqli_num_rows($result_a)>0){
     while($row_a=mysqli_fetch_assoc($result_a)){
        $bkash=$row_a['Bkash_Number'];
        $nogod=$row_a['Nogod_Number'];
         }

    }

if(  $pay_option=='bkash'){
    echo"<h3  class='text-danger m-5 '>Please !! Complete your payment on this Bkash Number : $bkash .</h3>";
   
}
else{
    echo"<h3  class='text-danger'>Please !! Complete your payment on this Nogod Number : $nogod .</h3>";
}
    ...
    <div class="col-md-2"></div>
            <div class="col-md-8">
                  <form action="" method="post">
                      ...
                      <input type="text" name="tr_num" class="form-control" placeholder="Enter Your payment transaction number" id="text">
                      </div>
                      <button type="submit" name="con_payment" class="btn btn-primary">Submit</button>
                  </form>
...
$crat="CREATE TABLE con_payment(
    Cust_Name varchar(25),
    Cust_nid varchar(25),
    tr_num varchar(25),
    tr_method varchar(25) NULL,
    tr_date varchar(25) NULL,
    p_id varchar(25) NULL,
    pay_status varchar(25) NULL
    )";
